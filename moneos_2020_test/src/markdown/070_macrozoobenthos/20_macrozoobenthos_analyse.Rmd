...
data_macrobenthos_totaal <-
  ...
  summarise_at(vars(densiteit, biomassa), ~sum(.)) %>% 
  ungroup()
...
```{r 070-figuur-densiteit-waterlichaam-alternatief}
...
data_macrobenthos_totaal %>% 
  ...
  summarise(leeg = sum(densiteit == 0 & biomassa == 0),
            totaal = n(),
